...
class Event extends CI_Controller {
	...
	// public function class($id_event = null)
	// {
	// 	$id_eo = $_SESSION['id_eo'];
	// 	$data = array(
	// 		'page' => 'event/class',
	// 		'data' => $this->db->where('id_eo', $_SESSION['id_eo'])->where('flag',1)->get('tbl_class')->result(),
	// 	  );
	...
	public function template_design($id = null){
		...
		$data = array(
			...
			// 'data'=> $this->db->where('id_eo', $id_eo)->where('public_date !=', null )->get('tbl_template')->result(),
		  );
...
public function insert_event($id_event = null){
	...
	// $data_mku_template = $this->Template_model->get_all_mku($id_eo);
